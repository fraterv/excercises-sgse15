Copyright by Michael, Philipp and Erwin
...
(function () {
    ...
    var querySlidingHtml = function querySlidingHtml() {
        ...
        for (j = 0, jlen = sections.length; j < jlen; j++) {
            ...
            if (section.getAttribute('data-html').length) {
                ...
                var xhr = new XMLHttpRequest(),
                    ...
                    cb = function cb() {
                    if (xhr.readyState === 4) {
                        if (xhr.status >= 200 && xhr.status < 300 || xhr.status === 0 // file protocol yields status code 0 (useful for local debug, mobile applications etc.)
                        ) {
